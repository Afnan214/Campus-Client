...
const AllStudentsContainer = () => {
  const allStudents = useSelector((state) => state.allStudents.students)
  const dispatch = useDispatch()
  useEffect(() => {
    const fetchAllStudents = async () => {
      const res = await fetchAllStudentsThunk()
      console.log(res)
      return res;
    }
    const res = fetchAllStudents()
    res.then((res) => {
      dispatch(allStudentsFetched(res))
    })
  }, [])

  const thisdeleteStudent = (id) => {
    console.log("deleting")
    const deleteStudentAsync = async () => {
      const res = await deleteStudentThunk(id);
      return res
    }
    const res = deleteStudentAsync();
    res.then((res) => {
      dispatch(allStudentDeleted(res))
    })
  }
  return (
    <>
      <Header />
      <AllStudentsView students={allStudents} deleteStudent={(id) => thisdeleteStudent(id)} />
    </>
